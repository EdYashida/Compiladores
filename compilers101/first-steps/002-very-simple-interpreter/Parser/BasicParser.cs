...
namespace VerySimpleInterpreter.Parser
{
    public class BasicParser
    {
        ...
        {
            this. _lexer = lexer;
            this._lookAhead = _lexer.GetNextToken();
            this._symbolTable = st;
        }

        public void Match(ETokenType type)
        {
            if (_lookAhead.Type == type)           //se tiver mesmo tipo do token
                _lookAhead = _lexer.GetNextToken(); //passa pro proximo token
            else
                Error("Era esperado um " + type + " - E foi encontrado um " + _lookAhead.Type); //se nao
        }

        public void Error(String msg)
        {
            Console.WriteLine("Erro em _____ "+_lookAhead.Type+" "+_lookAhead.Value);
            Console.WriteLine("Linha " + _lexer.Line);
            Console.WriteLine("Coluna/posicao " + _lexer.Column);
        ...
        {       //analisa linha a linha
            Stmt();
            if(_lookAhead.Type == ETokenType.EOF){
            Match(ETokenType.EOF);  //tratar loop inifinito, reconhecendo fim do arquivo
            }
            else{
                Match(ETokenType.ERR); //o mesmo, caso haja erro de sintaxe, gerando erro
            }                                    
        }
        ...
        public void Stmt() //stmt   : in | out | atrib  
        {
            if (_lookAhead.Type == ETokenType.INPUT)
                ...
                Expr();
            else if (_lookAhead.Type == ETokenType.EOL)
                Match(ETokenType.EOL);
            else
                Error("Erro, era esperado um INPUT, OUTPUT , VAR , NUM ou EOL");
        }
        ...
        {
            if (_lookAhead.Type == ETokenType.NUM){  //numeros
                Double num = Convert.ToDouble(_lookAhead.Value);
                Match(ETokenType.NUM);
                return num;
            }
                
            else if (_lookAhead.Type == ETokenType.VAR){  //letras
                int? key = _lookAhead.Value;
                Double val = 0;

                if (key == null)
                {
                    Error("Era esperado um VAR");
            ...
            else if (_lookAhead.Type == ETokenType.OE) //parenteses
            {
                Match(ETokenType.OE);
                var e = Expr();
                Match(ETokenType.CE);
